...
class manipulation:
    ...
    def tratamento_base(self):
        ...
        self.df_base_copia = self.__criando_indicadores_base()
        return self.df_base_copia
    ...
    #def __tratamento_inicial_base(self):
        #try:
            ...
            #rint('tratamento inicial base - sucesso')
        #except:
    ...
    def __criando_indicadores_base(self):
        #try:
            ...
            def flag_clientes(df):
                if df['dias_ult_comp'] <=75:
                    return 'Ativo'
                elif ((df['dias_ult_comp'] >75)&(df['dias_ult_comp']<=90)):
            ...
            self.df_base_copia = self.df_base.loc[self.df_base["flag"]=='Proximo a inativar'].copy().reset_index(drop=True)
            self.df_base_copia.drop(['temp_empresa'], axis=1, inplace=True)
    ...
    def lista_emails_agg(self):
        lista_emails = self.df_emails['emails'].tolist()
        return lista_emails
    ...
    def __merge_emails_agg(self):
        try:
            ...
            self.df_agg.drop(columns=['index'], inplace=True)
